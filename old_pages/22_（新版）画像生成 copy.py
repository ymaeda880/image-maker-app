...
if st.button("生成する", use_container_width=True):
    ...
    st.image(png_bytes, caption="生成結果", use_container_width=True)
...
st.image(st.session_state["simple_last_png"], caption="現在の元画像", use_container_width=True)
...
if st.button("修正版を生成する（修正内容のプロンプトを反映します）", use_container_width=True):
    ...
    with st.spinner("修正版を生成中..."):
        ...
        st.image(out_bytes, caption="修正版（次の元画像）", use_container_width=True)
...
if png_bytes:
    ...
    # ダウンロードボタン
    default_name = f"generated_{dt.datetime.now():%Y%m%d_%H%M%S}.png"
