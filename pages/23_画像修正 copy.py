# -*- coding: utf-8 -*-
# toolkit_app/pages/23_画像修正.py
# ============================================================
# 🧪 テンプレ寄せ：画像アップロード（or Inbox選択）→ 何度でも修正（gpt-image-1）
#
# 方針（テンプレ準拠）
...
# セッションキー（ページ専用）
# ============================================================
K_LAST_PNG = f"{PAGE_NAME}__last_png"               # 現在の修正対象PNG（常に最新）
K_UPLOADED_PNG = f"{PAGE_NAME}__uploaded_png"       # 読み込み直後PNG（初期元画像）

K_EDIT_PROMPT = f"{PAGE_NAME}__edit_prompt"

K_SIZE = f"{PAGE_NAME}__size"                       # ★ 出力サイズ（ボタン1組）
...
K_INBOX_SELECTED_ITEM = f"{PAGE_NAME}__inbox_selected_item"  # item_id
...
def _query_inbox_images_page(user_sub: str, limit: int, offset: int) -> tuple[list[dict[str, Any]], int]:
    ...
    try:
        cur = con.cursor()
        cur.execute("SELECT COUNT(1) FROM inbox_items WHERE kind = ?", ("image",))
...
# ページ設定
# ============================================================
st.set_page_config(page_title="Image Maker / 画像修正", page_icon="🧪", layout="wide")
...
# 1-b) Inbox から読み込む（任意）
# ============================================================
use_inbox = st.toggle("📥 Inbox から読み込む（画像のみ）", value=False)
if use_inbox:
    if not INBOX_ROOT.exists():
        ...
        rows, total = _query_inbox_images_page(user_sub=str(sub), limit=INBOX_PAGE_SIZE, offset=offset)
        ...
        if total <= 0 or not rows:
            ...
            with nav1:
                if st.button("⬅ 前へ", disabled=(page_index <= 0), key=f"{PAGE_NAME}__inbox_prev"):
                    st.session_state[K_INBOX_PAGE] = max(page_index - 1, 0)
                    _clear_inbox_selection()
                    st.rerun()
            with nav2:
                if st.button("次へ ➡", disabled=(page_index >= last_page), key=f"{PAGE_NAME}__inbox_next"):
            ...
            with cbtn1:
                load_inbox_clicked = st.button("📥 Inbox 画像を読み込む", key=f"{PAGE_NAME}__load_from_inbox")
            with cbtn2:
